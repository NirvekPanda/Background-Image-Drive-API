...
func main() {
	...
	var folderID, mapsAPIKey, cloudSQLConnName, cloudSQLDatabase, cloudSQLUser, cloudSQLPassword string
	...
	// Create HTTP handler with direct service access
	handler := handlers.NewDirectHTTPHandler(imageService, locationService)
	...
	if secretManager != nil {
		...
		if corsOrigins != "" {
			...
			fmt.Printf("CORS origins from Secret Manager: %v\n", origins)
		}
	} else {
		fmt.Printf("Using default CORS origins: %v\n", corsConfig.AllowedOrigins)
	}
